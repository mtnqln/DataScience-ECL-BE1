...
def test_browsers_per_player_unique():
...
def test_browsers_per_player_not_unique():
    df = pd.DataFrame([["sph","Firefox"],["abc","Chrome"],["tre","Firefox"],["sph","Firefox"]]) 
    result = browsers_per_player(df=df,unique=False)
    expected_result = pd.DataFrame([["sph",1.0,0.0],["abc",0.0,1.0],["tre",1.0,0.0],["sph",1.0,0.0]], columns=[0,"Firefox","Chrome"])
    print("result :",result)
